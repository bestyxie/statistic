...
export default function Dashboard() {
  ...
  const changePercent = data.yesterday > 0 ? (((data.today - data.yesterday) / data.yesterday) * 100).toFixed(1) : '-'
        ...
        {data.trend.length > 0 ? (
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={data.trend}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="visitors" stroke="#3b82f6" strokeWidth={2} dot={{ r: 4 }} name="访客数" />
            </LineChart>
          </ResponsiveContainer>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-2 text-gray-500 font-medium">商品</th>
                  <th className="text-right py-3 px-2 text-gray-500 font-medium">浏览次数</th>
                  <th className="text-right py-3 px-2 text-gray-500 font-medium">浏览人数</th>
                </tr>
              </thead>
              <tbody>
