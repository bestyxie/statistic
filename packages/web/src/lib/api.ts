...
export const api = {
  ...
  getLabelTxTrend: (labelIds: string[], start: string, end: string, shopId?: string) =>
    request<{ items: LabelTxTrendItem[] }>(
      `/stats/label-tx-trend?label_ids=${labelIds.join(',')}&start=${start}&end=${end}${shopId ? `&shop_id=${shopId}` : ''}`,
    ),
  getLabelProducts: (labelId: string, date: string, shopId?: string) =>
    request<{ items: LabelProductStat[] }>(
      `/stats/label-products?label_id=${labelId}&date=${date}${shopId ? `&shop_id=${shopId}` : ''}`,
    ),
  getLabelSales: (start?: string, end?: string, shopId?: string) =>
    request<{ items: LabelSalesItem[] }>(
      `/stats/label-sales?${start ? `start=${start}&` : ''}${end ? `end=${end}&` : ''}${shopId ? `shop_id=${shopId}` : ''}`,
    ),
  getLabelSalesProducts: (labelId: string, start?: string, end?: string, shopId?: string) =>
    request<{ items: LabelSalesProduct[] }>(
      `/stats/label-sales-products?label_id=${labelId}${start ? `&start=${start}` : ''}${end ? `&end=${end}` : ''}${shopId ? `&shop_id=${shopId}` : ''}`,
    ),
}
