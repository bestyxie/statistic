...
import type { SupplierProduct, PurchaseRecord } from '@statistic/shared'
...
interface Props {
  ...
  onSubmit: (data: { supplier_product_id: string; price: string; quantity: number; purchase_date: string; note: string }) => Promise<void>
  onCancel: () => void
}

export default function PurchaseRecordForm({ products, purchase, onSubmit, onCancel }: Props) {
  const [form, setForm] = useState({
    supplier_product_id: purchase?.supplier_product_id || products[0]?.id || '',
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
        <div>
          ...
          >
            ...
            {products.map((p) => (
              <option key={p.id} value={p.id}>{p.description || p.product_code || p.id.slice(0, 8)}</option>
            ))}
          </select>
        </div>
        <div>
